...
//********** This section will read in deltas from a text file and store in variable ********
    ...
    std::string activation_function = argv[9];
    char str_delta[60];
    long int_delta;
    char *stopstring;
...
//        std::cout << "Training has been running for " << duration << " seconds" << std::endl;
        ...
        torch::Tensor outputTensor_test = model->forward(inputTest, activation_function, layers);
        sample_size++;
...
//    std::cout << "MSE: " << loss.item<float>() << std::endl << std::endl;
...
//    std::cout << "Calculations finished!\n\n";
    ...
    // Line to be appended:
    std::cout << "Trace: " << trace << " Num_inputs: " << argv[3] << " Num_layers: " << argv[5] << " Hidden_size: " << argv[6] << " Learning_rate: " << argv[7] << " Time_limit: " << argv[8] << " Act_func: " << argv[9] << " Window_size: " << argv[10] << " Acc: " << accuracy << std::endl;
